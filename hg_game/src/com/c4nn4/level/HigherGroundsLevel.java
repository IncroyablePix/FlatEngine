...
import com.c4nn4.level.entities.Cat;
import com.c4nn4.level.entities.Pickup;
import com.c4nn4.level.entities.Player;
import com.c4nn4.level.entities.Sushi;
import com.c4nn4.level.entities.controlables.PlayerJump;
import com.c4nn4.level.entities.particles.Particles;
import com.c4nn4.level.entities.particles.types.ParticleFabric;
import com.c4nn4.level.map.MapGenerator;
import com.c4nn4.level.map.Mapping;
import com.c4nn4.level.map.RandomGenerator;
import com.c4nn4.pix_engine.main.GameThread;
import com.c4nn4.pix_engine.main.screen.Screen;
import com.c4nn4.game.level.GameLevel;
...
import static com.c4nn4.pix_engine.main.screen.Screen.WIN_WIDTH;
import static com.c4nn4.pix_engine.manager.image.PixManager.TILE_SIZE;

public class HigherGroundsLevel implements GameLevel {
    ...
    public HigherGroundsLevel(int level) {
        loadImages();
        this.level = level;
        ...
        if(!am.isLoaded("ingame") || !am.isPlaying("ingame")) {
            am.load("/com/c4nn4/resources/Sound/Music/ambiantlow.mp3", "ingame");
            am.setVolume("ingame", -10);
            am.loop("ingame");
        }

        Debug.log("Creating map...");
        this.map = new Mapping(BOARD, level * 4 + 20, camera, new RandomGenerator(level, BOARD));
    ...
    private void finish() {
        ...
        am.load("/com/c4nn4/resources/Sound/SFX/cat" + MapGenerator.randomEx(1, 6) + ".wav", "cat");
        am.setVolume("cat", 0);
        am.play("cat");
    ...
    public void onPlayerReachesPlatform(final Player player, final Platform platform) {
        if (platform != null) {
            //ParticleFabric.createParticles(ParticleFabric.ParticleType.PASCAL_OP_PARTICLE, player.getX(), player.getY(), 10, 300,false, this);
    ...
    private void updateScrolling() {
        ...
        double height = HigherGroundsLevel.BOARD_HEIGHT;
        double width = HigherGroundsLevel.BOARD;
